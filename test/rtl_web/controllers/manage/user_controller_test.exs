defmodule RTLWeb.Manage.UserControllerTest do
  ...
  describe "#create" do
    test "inserts the user and redirects", %{conn: conn} do
      ...
      conn = post(conn, Routes.manage_user_path(conn, :create), params)
      ...
      assert redirected_to(conn) == Routes.manage_user_path(conn, :show, user.id)
    end
  ...
  describe "#update" do
    test "saves changes and redirects", %{conn: conn} do
      ...
      conn = patch(conn, Routes.manage_user_path(conn, :update, user.id), params)

      updated = Accounts.get_user!(user.id)
      assert updated.full_name == "Daffy"
      assert updated.email == "daffy@duck.com"
      assert redirected_to(conn) == Routes.manage_user_path(conn, :show, user)
    end
    ...
    test "rejects changes if invalid", %{conn: conn} do
      ...
      conn = patch(conn, Routes.manage_user_path(conn, :update, user), params)
  ...
  describe "#delete" do
    test "deletes the user", %{conn: conn} do
      ...
      assert redirected_to(conn) == Routes.manage_user_path(conn, :index)
    end
  end
end
