defmodule RTLWeb.Manage.VideoImportControllerTest do
  use RTLWeb.ConnCase, async: true

  test "all actions require project admin", %{conn: conn} do
    project = Factory.insert_project()
    conn = get(conn, Routes.manage_video_import_path(conn, :new, project))
  ...
  describe "#create" do
    test "imports the videos", %{conn: conn} do
      ...
      conn = post(conn, Routes.manage_video_import_path(conn, :create, project), params)
    ...
    defp stub_videos_json, do: ~s([
      {"permission":null,"recording_filename":"video1.webm","source_name":"YouTube","source_url":"youtube.com/1","thumbnail_filename":"thumb1.jpg","title":"Computer science"},
      {"permission":null,"recording_filename":"video2.webm","source_name":"YouTube","source_url":"youtube.com/2","thumbnail_filename":"thumb2.jpg","title":"Pride standing in the way of progress"},
      {"permission":null,"recording_filename":"video3.webm","source_name":"YouTube","source_url":"youtube.com/3","thumbnail_filename":"thumb3.jpg","title":"Core values"}
    ])
  end
end
