defmodule RTLWeb.Admin.PromptControllerTest do
  use RTLWeb.ConnCase, async: true
  alias RTL.Projects

  defp prompt_path(conn, action, proj) do
    Routes.admin_prompt_path(conn, action, proj)
  end

  defp prompt_path(conn, action, proj, prompt) do
    Routes.admin_prompt_path(conn, action, proj, prompt)
  end
  ...
  describe "#create" do
    test "inserts the prompt and redirects", %{conn: conn} do
      ...
      assert redirected_to(conn) == Routes.admin_project_path(conn, :show, proj)
    end
