defmodule RTLWeb.Auth do
  ...
  def load_current_user(conn, _opts) do
    cond do
      ...
      # Must come after the expiration check
  ...
  def try_login(conn, email, pw, remember_me: remember_me) do
    ...
    cond do
      ...
      # user is found, but pw doesn't match
      user ->
        {:error, :unauthorized, conn}

      # user isn't found
      true ->
        # prevent timing attacks that could derive valid emails
        dummy_checkpw()
  ...
  defp session_expired?(conn) do
    ...
    remember_me != "true" && (expires_at == nil || is_future?(parse_time(expires_at)))
  end

  defp parse_time(string), do: Timex.parse!(string, "{ISO:Extended}")

  defp is_future?(time), do: Timex.after?(Timex.now(), time)
